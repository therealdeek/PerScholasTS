...
const runDashboard = () => {
    ...
    retryPromise(() => fetchProductCatalog(), RETRIES, RETRY_DELAY)
        ...
        const reviewPromises = products.map((product) => retryPromise(() => fetchProductReviews(product.id), RETRIES, RETRY_DELAY)
            .then((reviews) => {
            ...
            if (error instanceof NetworkError) {
                ...
                console.error(`[DataError on field "${error.field}"] ${error.message}`);
            }
            else {
